...
class FeatureExtractor:
    ...
    def extract_team_offensive_features(self, team_id: int, before_date: date) -> dict:
        ...
        if len(game_ids) < 3:
            ...
            raise InsufficientDataError(
                f"Team {team_id} has less than 3 games before {before_date}"
            )

        # Aggregate team stats from these games
        stats = FootballTeamGameStat.objects.filter(
            team_id=team_id, game_id__in=game_ids
        ).aggregate(
            # Passing stats
        ...
        return {
            ...
            "off_total_yards": (stats["avg_pass_yards"] or 0)
            + (stats["avg_rush_yards"] or 0),
            "off_points_scored": avg_points,
            "off_turnovers": (stats["avg_interceptions"] or 0)
            + (stats["avg_fumbles_lost"] or 0),
        }
    ...
    def extract_team_defensive_features(self, team_id: int, before_date: date) -> dict:
        ...
        opponent_stats = (
            FootballTeamGameStat.objects.filter(game_id__in=game_ids)
            .exclude(team_id=team_id)  # Exclude this team's own stats
            .aggregate(
    ...
    def extract_situational_features(self, game: Game, team_id: int) -> dict:
        ...
        return {
            ...
            "temperature": (
                game.temp if game.temp and not is_dome else 70
            ),  # Default to 70°F for domes
